...
# To check your region, replace us-west-2 with your region and run the below command.
# AWS_REGION=us-west-2 && ~/.local/bin/aws ec2 describe-instance-type-offerings --query "InstanceTypeOfferings[?Location=='$AWS_REGION'] [InstanceType] | sort_by(@, &[0])" --filter Name="instance-type",Values="t4g.*,a1.*" --region $AWS_REGION --output text
instance_type = "t4g.micro"

# The Ubuntu ARM AMI name string, these are occasionally updated with a new date.
# To find your region's AMI, replace us-west-2 with your region, then run the command:
# For ARM:
# AWS_REGION=us-west-2 && ~/.local/bin/aws ec2 describe-images --region $AWS_REGION --owners 099720109477 --filters 'Name=name,Values=ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-arm64-server-*' 'Name=state,Values=available' --query 'sort_by(Images, &CreationDate)[-1].Name'
# For x86_64 (non-ARM):
# AWS_REGION=us-west-2 && ~/.local/bin/aws ec2 describe-images --region $AWS_REGION --owners 099720109477 --filters 'Name=name,Values=ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-amd64-server-*' 'Name=state,Values=available' --query 'sort_by(Images, &CreationDate)[-1].Name'
